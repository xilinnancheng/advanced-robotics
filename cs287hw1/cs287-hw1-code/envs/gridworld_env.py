...
class GridWorldEnv(DiscreteEnv):
    ...
    def __init__(self, size=20, discount=0.99, seed=0):
        ...
        self._grid[0, 0], self._grid[1, 0], self._grid[0,
                                                       1], self._grid[1, 1] = 0, 0, 0, 0
        self._grid[-1, -1], self._grid[-2, -1], self._grid[-1, -
                                                           2], self._grid[-2, -2] = 0, 0, 0, 0
        self._rgb_grid = np.zeros((size, size, 3), dtype=np.uint8)
        self._rgb_grid[:, :, :] = np.expand_dims(
            ((1-self._grid) * 255).astype(np.uint8), axis=-1)
    ...
    def vec_reset(self, num_states):
        states = np.random.randint(
            0, self._size * self._size, size=(num_states,))
        collisions = self._grid[states % self._size, states // self._size]
        num_collisions = np.sum(collisions)
        while num_collisions:
            states[collisions.astype(bool)] = np.random.randint(
                0, self._size * self._size, size=(num_collisions,))
            collisions = self._grid[states % self._size, states // self._size]
            num_collisions = np.sum(collisions)
        self._states = states
        return self._states
