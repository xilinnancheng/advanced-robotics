...
class ValueIteration(object):
    ...
    def train(self):
        ...
        self.policy.update(next_pi)
        contour, fig = plot_contour(
            self.env, self.value_fun, save=True, fig=fig, iteration=itr)
    ...
    def get_next_values(self):
        ...
        if self.policy_type == 'deterministic':
            ...
            # please refer to https://zhuanlan.zhihu.com/p/31275071 for the meaning of axis
            ...
            q = np.multiply(self.transitions,
                            self.rewards + self.discount * self.value_fun.get_values()).sum(axis=2)
            # next_v = self.temperature * \
            #     np.log(np.sum(np.exp(1 / self.temperature * q), axis=1))
            q_soft_max = np.log(
                np.exp(1.0/self.temperature * (q - q.max(axis=1, keepdims=True))).sum(axis=1))
            next_v = self.temperature * q_soft_max + q.max(axis=1)
            """ Your code ends here"""
        else:
    ...
    def _stop_condition(self, itr, next_v, v):
        ...
        cond1 = np.max(np.abs(next_v - v)) < self.precision / \
            (2 * self.discount/(1 - self.discount))
        cond2 = self.discount ** itr * rmax / \
            (1 - self.discount) < self.precision
