...
def grad_step(loss, params, lr):
    ...
    new_params = dict([(k, params[k] - lr * gradient[k])
...
class TabularPolicy(object):
    def __init__(self, env):
        ...
        self._policy = np.random.uniform(
            0, 1, size=(self.obs_dim, self.act_dim))
...
class SparseArray(object):
    ...
    def __mul__(self, other):
        if isinstance(other, SparseArray):
            assert (self._idxs == other._idxs).all(
            ), "other does not have the same sparsity"
            result = SparseArray(self._obs_n, self._act_n,
                                 self._mode, self._obs_dims)
        ...
        if type(key) is not tuple:
            ...
            if self._mode == 'nn':
                ...
                if isinstance(n_obs, np.ndarray) and n_obs.ndim == 2:
                    ...
                    if value.ndim == 1:
                        self._values[obs, act, :] = np.expand_dims(
                            value, axis=-1)
                    else:
                        self._values[obs, act, :] = value
    ...
    def __getitem__(self, key):
        if type(key) is not tuple:
            ...
            if self._mode == 'nn':
                ...
                assert (n_obs == self._idxs[obs,
                        act, self._fill[obs, act]]).all()
...
class DiscretizeWrapper(object):
    ...
    def vec_step(self, ids_a):
        ...
        next_obs, rewards, dones, env_info = self._wrapped_env.vec_step(
            actions)
        if hasattr(self, 'get_discrete_state_from_cont_state'):
            ...
            id_next_s, probs = self.vec_get_discrete_state_from_cont_state(
                next_obs)
    ...
    def get_discretization(self, discretization, low, high):
        ...
        if type(discretization) is int:
            ...
            points = np.stack([np.linspace(l, h, discretization)
            ...
            if not isinstance(discretization[0], Iterable):
                ...
                points = np.ones((dim, np.max(discretization))
                                 ) * (high[:, None] + 1e3)
    ...
    def _build(self):
        ...
        if isinstance(env.observation_space, spaces.Discrete):
            ...
            self._state_base_transf = np.cumprod(
                np.concatenate([[1], self._state_bins_per_dim[:-1]]))
            self._all_coordinates = self.get_coordinates_from_id(
                np.arange(2 ** self.obs_dims), base=2).T
...
class DiscreteEnv(Env):
    ...
    def __init__(self, obs_dim, act_dim):
        ...
        assert (np.sum(self._transitions, axis=-1)
                == np.ones((obs_dim, act_dim))).all
        self._state = 0
...
class VectorizeMujocoEnv(object):
    ...
    def _create_envs(self):
        ...
        self.envs_per_proc = [
            int(self._num_envs/self.n_parallel)] * self.n_parallel
        ...
        self.remotes, self.work_remotes = zip(
            *[Pipe() for _ in range(self.n_parallel)])
        self.ps = [
            Process(target=worker, args=(work_remote, remote,
                    pickle.dumps(self._wrapped_env), n_envs))
    ...
    def vec_step(self, actions):
        ...
        obs, rewards, dones, env_infos = map(
            lambda x: sum(x, []), zip(*results))
